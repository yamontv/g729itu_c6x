...
/* local functions */
static void Qnt_e(const Word16 *errlsf, /* (i)  : error lsf vector             */
                  const Word16 *weight, /* (i)  : weighting vector             */
                  Word16 *qlsf, /* (o)  : quantized error lsf vector   */
                  Word16 *Pptr, /* (o)  : predictor index              */
                  Word16 *cluster /* (o)  : quantizer indices            */
                  );

static void New_ML_search_1(const Word16 *d_data, /* (i) : error vector             */
                            Word16 *new_d_data,/* (o) : output vector            */
                            Word16 *best_indx, /* (o) : best indices             */
                            Word16 *ptr_back, /* (o) : pointer for backtracking */
                            const Word16 *PtrTab /* (i) : quantizer table          */
                            );

static void New_ML_search_2(const Word16 *d_data, /* (i) : error vector             */
                            const Word16 *weight, /* (i) : weighting vector         */
                            Word16 *new_d_data,/* (o) : output vector            */
                            Word16 *best_indx, /* (o) : best indices             */
                            const Word16 *ptr_prd, /* (i) : pointer for backtracking */
                            Word16 *ptr_back, /* (o) : pointer for backtracking */
                            const Word16 PtrTab[2][16] /* (i) : quantizer table        */
                            );
                ...
                Word16 *ana)
{
    ...
    Word16 Clust[MODE], mode, errlsf[M * MODE];
    ...
    Lsp_prev_compose(tmpbuf, lsfq, (const Word16 (*)[M]) noise_fg[mode],
                     (const Word16 (*)[M]) freq_prev, noise_fg_sum[mode]);
                  ...
                  )
{
    ...
    /* MS[0] = 32 */
    New_ML_search_1(errlsf, d_data[0], best_indx[0], ptr_back[0], PtrTab_1);
    /* MS[1] = 16 */
    New_ML_search_2(d_data[0], weight, d_data[1], best_indx[1], ptr_back[0],
                    ptr_back[1], PtrTab_2);
                            ...
                            )
{
    ...
    for (p = 0; p < 2; p++)
        for (m = 0; m < 32; m++)
        {
            ...
            sum[p * 32 + m] = extract_h(acc0);
            sum[p * 32 + m] = mult(sum[p * 32 + m], Mp[p]);
        }
    ...
    for (q = 0; q < 4; q++)
    {
        ...
        sum[min_indx_p[q] * 32 + min_indx_m[q]] = MAX_16;
    }
                            ...
                            )
{
    ...
    for (p = 0; p < 4; p++)
        for (m = 0; m < 16; m++)
        {
            ...
            for (l = 0; l < M / 2; l++)
            {
                tmp1 =
                        extract_h(
                                L_shl( L_mult(noise_fg_sum[ptr_prd[p]][l], noise_fg_sum[ptr_prd[p]][l]), 2));
    ...
    for (q = 0; q < 1; q++)
    {
        ...
        sum[min_indx_p[q] * 16 + min_indx_m[q]] = MAX_16;
    }
