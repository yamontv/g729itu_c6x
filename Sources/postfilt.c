...
static void pit_pst_filt(Word16 *signal, /* (i)     : input signal                        */
...
static void preemphasis(Word16 *signal, /* (i/o)   : input signal overwritten by the output */
Word16 g, /* (i) Q15 : preemphasis coefficient                */
Word16 L, /* (i)     : size of filtering                      */
Word16 *mem_pre);

static void agc(Word16 *sig_in, /* (i)     : postfilter input signal  */
...
void Init_Post_Filter(struct postfilt_ctx *postfc) {
	postfc->res2 = postfc->res2_buf + PIT_MAX;
	postfc->scal_res2 = postfc->scal_res2_buf + PIT_MAX;

	Set_zero(postfc->mem_syn_pst, M);
	Set_zero(postfc->res2_buf, PIT_MAX + L_SUBFR);
	Set_zero(postfc->scal_res2_buf, PIT_MAX + L_SUBFR);

	postfc->past_gain = 4096; /* past_gain = 1.0 (Q12) */
	postfc->mem_pre = 0;
...
Word16 Vad) {
	...
	for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
	{
		...
		Weight_Az(Az, GAMMA2_PST, Ap3);
		Weight_Az(Az, GAMMA1_PST, Ap4);

		/* filtering of synthesis speech by A(z/GAMMA2_PST) to find res2[] */

		Residu(Ap3, &syn[i_subfr], postfc->res2);

		/* scaling of "res2[]" to avoid energy overflow */

		for (j = 0; j < L_SUBFR; j++) {
			postfc->scal_res2[j] = shr(postfc->res2[j], 2);
		}

		/* pitch postfiltering */
		if (sub(Vad, 1) == 0)
			pit_pst_filt(postfc->res2, postfc->scal_res2, t0_min, t0_max,
			L_SUBFR, res2_pst);
		else
			for (j = 0; j < L_SUBFR; j++)
				res2_pst[j] = postfc->res2[j];
		...
		Syn_filt_for_post(Ap4, h, h, &h[M + 1], 0);
		...
		preemphasis(res2_pst, temp2, L_SUBFR, &postfc->mem_pre);

		/* filtering through  1/A(z/GAMMA1_PST) */

		Syn_filt(Ap4, res2_pst, &syn_pst[i_subfr], postfc->mem_syn_pst,
				1);

		/* scale output to input */

		agc(&syn[i_subfr], &syn_pst[i_subfr], L_SUBFR, &postfc->past_gain);

		/* update res2[] buffer;  shift by L_SUBFR */

		Copy(&postfc->res2[L_SUBFR - PIT_MAX], &postfc->res2[-PIT_MAX],
		PIT_MAX);
		Copy(&postfc->scal_res2[L_SUBFR - PIT_MAX],
				&postfc->scal_res2[-PIT_MAX], PIT_MAX);
...
Word16 *mem_pre) {
	...
	for (i = 0; i <= L - 2; i++) {
		/* fix pointer decrement by Slavka */
		*p1 = sub(*p1, mult(g, *p2--));
		p1--;
	}
