...
static void Lsp_qua_cs(Word16 qua_freq_prev[MA_NP][M], Word16 flsp_in[M], /* (i) Q13 : Original LSP parameters    */
Word16 lspq_out[M], /* (o) Q13 : Quantized LSP parameters   */
Word16 *code /* (o)     : codes of the selected LSP  */
);

static void Relspwed(const Word16 lsp[], /* (i) Q13 : unquantized LSP parameters */
const Word16 wegt[], /* (i) norm: weighting coefficients     */
Word16 lspq[], /* (o) Q13 : quantized LSP parameters   */
const Word16 lspcb1[][M], /* (i) Q13 : first stage LSP codebook   */
const Word16 lspcb2[][M], /* (i) Q13 : Second stage LSP codebook  */
const Word16 fg[MODE][MA_NP][M], /* (i) Q15 : MA prediction coefficients */
Word16 qua_freq_prev[MA_NP][M], /* (i) Q13 : previous LSP vector        */
const Word16 fg_sum[MODE][M], /* (i) Q15 : present MA prediction coef.*/
const Word16 fg_sum_inv[MODE][M], /* (i) Q12 : inverse coef.              */
Word16 code_ana[] /* (o)     : codes of the selected LSP  */
);

static void Lsp_pre_select(const Word16 rbuf[], /* (i) Q13 : target vetor             */
const Word16 lspcb1[][M], /* (i) Q13 : first stage LSP codebook */
Word16 *cand /* (o)     : selected code            */
);

static void Lsp_select_1(const Word16 rbuf[], /* (i) Q13 : target vector             */
const Word16 lspcb1[], /* (i) Q13 : first stage lsp codebook  */
const Word16 wegt[], /* (i) norm: weighting coefficients    */
const Word16 lspcb2[][M], /* (i) Q13 : second stage lsp codebook */
Word16 *index /* (o)     : selected codebook index   */
);

static void Lsp_select_2(const Word16 rbuf[], /* (i) Q13 : target vector             */
const Word16 lspcb1[], /* (i) Q13 : first stage lsp codebook  */
const Word16 wegt[], /* (i) norm: weighting coef.           */
const Word16 lspcb2[][M], /* (i) Q13 : second stage lsp codebook */
Word16 *index /* (o)     : selected codebook index   */
);

static void Lsp_get_tdist(const Word16 wegt[], /* (i) norm: weight coef.                */
const Word16 buf[], /* (i) Q13 : candidate LSP vector        */
Word32 *L_tdist, /* (o) Q27 : distortion                  */
const Word16 rbuf[], /* (i) Q13 : target vector               */
const Word16 fg_sum[] /* (i) Q15 : present MA prediction coef. */
);

static void Lsp_last_select(const Word32 L_tdist[], /* (i) Q27 : distortion         */
...
Word16 ana[] /* (o)     : indexes                    */
	...
	Lsp_qua_cs(qua_freq_prev, lsf, lsf_q, ana);
...
void Lsp_encw_reset(Word16 qua_freq_prev[MA_NP][M]) {
	...
	for (i = 0; i < MA_NP; i++)
		Mcopy(&freq_prev_reset[0], &qua_freq_prev[i][0]);
}
...
Word16 *code /* (o)     : codes of the selected LSP  */
	...
	Relspwed(flsp_in, wegt, lspq_out, lspcb1, lspcb2, fg, qua_freq_prev, fg_sum,
			fg_sum_inv, code);
}
...
Word16 code_ana[] /* (o)     : codes of the selected LSP  */
	...
	for (mode = 0; mode < MODE; mode++) {
		Lsp_prev_extract(lsp, rbuf, fg[mode],
				(const Word16 (*)[M]) qua_freq_prev, fg_sum_inv[mode]);
	...
	Lsp_get_quant(lspcb1, lspcb2, cand[mode_index], tindex1[mode_index],
			tindex2[mode_index], fg[mode_index], qua_freq_prev, lspq,
			fg_sum[mode_index]);
}
