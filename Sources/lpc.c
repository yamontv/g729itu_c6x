...
void Levinson(struct dtx_ctx * restrict dtxc, Word16 Rh[], /* (i)     : Rh[M+1] Vector of autocorrelations (msb) */
Word16 Rl[], /* (i)     : Rl[M+1] Vector of autocorrelations (lsb) */
Word16 A[restrict], /* (o) Q12 : A[M]    LPC coefficients  (m = 10)       */
...
Word16 *Err /* (o)     : Residual energy                          */
	...
	for (i = 2; i <= M; i++) {
		...
		if (sub(abs_s(Kh), 32750) > 0) {
			...
			rc[0] = dtxc->old_rc[0]; /* only two rc coefficients are needed */
			rc[1] = dtxc->old_rc[1];
			return;
		}
	...
	*Err = shr(alp_h, alp_exp);
	...
	for (i = 1; i <= M; i++) {
		t0 = L_Comp(Ah[i], Al[i]);
		dtxc->old_A[i] = A[i] = g_round(L_shl(t0, 1));
	}
	dtxc->old_rc[0] = rc[0];
	dtxc->old_rc[1] = rc[1];
...
static inline Word16 Chebps_uni(Word16 x, Word16 f[], unsigned ovf_coef);

void Az_lsp(Word16 a[], /* (i) Q12 : predictor coefficients              */
Word16 lsp[restrict], /* (o) Q15 : line spectral pairs                 */
Word16 old_lsp[restrict] /* (i)     : old lsp[] (in case not found 10 roots) */
) {
	Word16 i, j, nf, ip;
	...
	xlow = grid[0];
	ylow = Chebps_uni(xlow, coef, ovf_coef);
	...
	while ((nf < M) && (j < GRID_POINTS)) {
		...
		if (L_temp <= (Word32) 0) {
			...
			for (i = 0; i < 2; i++) {
				...
				ymid = Chebps_uni(xmid, coef, ovf_coef);
...
//  t0 = Mpy_32_16(b1_h, b1_l, x);        /* t0 = x*b1;              */
    ...
    t0 = _sshl(t0, ovf_coef); /* Q23/Q24 to Q30 with saturation */
