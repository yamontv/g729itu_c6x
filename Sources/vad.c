...
/* local function */
static Word16 MakeDec(Word16 dSLE, /* (i)  : differential low band energy */
 ...
 *                                                                 *
 *-----------------------------------------------------------------*/
void vad(struct vad_ctx * restrict vadc, Word16 rc, Word16 * restrict lsf, Word16 *r_h, Word16 *r_l,
	...
	if (sub(frm_count, 129) < 0) {
		if (sub(ENERGY, vadc->Min) < 0) {
			vadc->Min = ENERGY;
			vadc->Prev_Min = ENERGY;
		}

		if ((frm_count & 0x0007) == 0) {
			i = sub(shr(frm_count, 3), 1);
			vadc->Min_buffer[i] = vadc->Min;
			vadc->Min = MAX_16;
		}
	}

	if ((frm_count & 0x0007) == 0) {
		vadc->Prev_Min = vadc->Min_buffer[0];
		for (i = 1; i < 16; i++) {
			if (sub(vadc->Min_buffer[i], vadc->Prev_Min) < 0)
				vadc->Prev_Min = vadc->Min_buffer[i];
		}
	}

	if (sub(frm_count, 129) >= 0) {
		if (((frm_count & 0x0007) ^ (0x0001)) == 0) {
			vadc->Min = vadc->Prev_Min;
			vadc->Next_Min = MAX_16;
		}
		if (sub(ENERGY, vadc->Min) < 0)
			vadc->Min = ENERGY;
		if (sub(ENERGY, vadc->Next_Min) < 0)
			vadc->Next_Min = ENERGY;

		if ((frm_count & 0x0007) == 0) {
			for (i = 0; i < 15; i++)
				vadc->Min_buffer[i] = vadc->Min_buffer[i + 1];
			vadc->Min_buffer[15] = vadc->Next_Min;
			vadc->Prev_Min = vadc->Min_buffer[0];
			for (i = 1; i < 16; i++)
				if (sub(vadc->Min_buffer[i], vadc->Prev_Min) < 0)
					vadc->Prev_Min = vadc->Min_buffer[i];
	...
	if (sub(frm_count, INIT_FRAME) <= 0) {
		if (sub(ENERGY, 3072) < 0) {
			...
			*marker = VOICE;
			acc0 = L_deposit_h(vadc->MeanE);
			acc0 = L_mac(acc0, ENERGY, 1024);
			vadc->MeanE = extract_h(acc0);
			acc0 = L_deposit_h(vadc->MeanSZC);
			acc0 = L_mac(acc0, ZC, 1024);
			vadc->MeanSZC = extract_h(acc0);
			for (i = 0; i < M; i++) {
				acc0 = L_deposit_h(vadc->MeanLSF[i]);
				acc0 = L_mac(acc0, lsf[i], 1024);
				vadc->MeanLSF[i] = extract_h(acc0);
			}
		}
	}
	...
	if (sub(frm_count, INIT_FRAME) >= 0) {
		if (sub(frm_count, INIT_FRAME) == 0) {
			acc0 = L_mult(vadc->MeanE, factor_fx[vadc->less_count]);
			acc0 = L_shl(acc0, shift_fx[vadc->less_count]);
			vadc->MeanE = extract_h(acc0);

			acc0 = L_mult(vadc->MeanSZC, factor_fx[vadc->less_count]);
			acc0 = L_shl(acc0, shift_fx[vadc->less_count]);
			vadc->MeanSZC = extract_h(acc0);

			for (i = 0; i < M; i++) {
				acc0 = L_mult(vadc->MeanLSF[i], factor_fx[vadc->less_count]);
				acc0 = L_shl(acc0, shift_fx[vadc->less_count]);
				vadc->MeanLSF[i] = extract_h(acc0);
			}

			vadc->MeanSE = sub(vadc->MeanE, 2048); /* Q11 */
			vadc->MeanSLE = sub(vadc->MeanE, 2458); /* Q11 */
		}

		dSE = sub(vadc->MeanSE, ENERGY);
		dSLE = sub(vadc->MeanSLE, ENERGY_low);
		dSZC = sub(vadc->MeanSZC, ZC);
		...
		if (vadc->flag == 1) {
			if ((pprev_marker == VOICE) && (prev_marker == VOICE)
					&& (*marker == NOISE)
					&& (sub(abs_s(sub(vadc->prev_energy, ENERGY)), 614) <= 0)) {
				vadc->count_ext++;
				*marker = VOICE;
				vadc->v_flag = 1;
				if (sub(vadc->count_ext, 4) <= 0)
					vadc->flag = 1;
				else {
					vadc->count_ext = 0;
					vadc->flag = 0;
		...
		if ((*marker == VOICE) && (sub(vadc->count_sil, 10) > 0)
				&& (sub(sub(ENERGY, vadc->prev_energy), 614) <= 0)) {
			*marker = NOISE;
			vadc->count_sil = 0;
		}
		...
		if ((sub(sub(ENERGY, 614), vadc->MeanSE) < 0)
				&& (sub(frm_count, 128) > 0) && (!vadc->v_flag)
				&& (sub(rc, 19661) < 0))
			*marker = NOISE;

		if ((sub(sub(ENERGY, 614), vadc->MeanSE) < 0) && (sub(rc, 24576) < 0)
				&& (sub(SD, 83) < 0)) {
			vadc->count_update++;
			if (sub(vadc->count_update, INIT_COUNT) < 0) {
				COEF = 24576;
				C_COEF = 8192;
				COEFZC = 26214;
				C_COEFZC = 6554;
				COEFSD = 19661;
				C_COEFSD = 13017;
			} else if (sub(vadc->count_update, INIT_COUNT + 10) < 0) {
				COEF = 31130;
				C_COEF = 1638;
				COEFZC = 30147;
				C_COEFZC = 2621;
				COEFSD = 21299;
				C_COEFSD = 11469;
			} else if (sub(vadc->count_update, INIT_COUNT + 20) < 0) {
				COEF = 31785;
				C_COEF = 983;
				COEFZC = 30802;
				C_COEFZC = 1966;
				COEFSD = 22938;
				C_COEFSD = 9830;
			} else if (sub(vadc->count_update, INIT_COUNT + 30) < 0) {
				COEF = 32440;
				C_COEF = 328;
				COEFZC = 31457;
				C_COEFZC = 1311;
				COEFSD = 24576;
				C_COEFSD = 8192;
			} else if (sub(vadc->count_update, INIT_COUNT + 40) < 0) {
				COEF = 32604;
				C_COEF = 164;
				COEFZC = 32440;
				C_COEFZC = 328;
			...
			/* compute MeanSE */
			acc0 = L_mult(COEF, vadc->MeanSE);
			acc0 = L_mac(acc0, C_COEF, ENERGY);
			vadc->MeanSE = extract_h(acc0);

			/* compute MeanSLE */
			acc0 = L_mult(COEF, vadc->MeanSLE);
			acc0 = L_mac(acc0, C_COEF, ENERGY_low);
			vadc->MeanSLE = extract_h(acc0);

			/* compute MeanSZC */
			acc0 = L_mult(COEFZC, vadc->MeanSZC);
			acc0 = L_mac(acc0, C_COEFZC, ZC);
			vadc->MeanSZC = extract_h(acc0);

			/* compute MeanLSF */
			for (i = 0; i < M; i++) {
				acc0 = L_mult(COEFSD, vadc->MeanLSF[i]);
		...
		if ((sub(frm_count, 128) > 0)
				&& (((sub(vadc->MeanSE, vadc->Min) < 0) && (sub(SD, 83) < 0))
						|| (sub(vadc->MeanSE, vadc->Min) > 2048))) {
			vadc->MeanSE = vadc->Min;
			vadc->count_update = 0;
		}
	}

	vadc->prev_energy = ENERGY;
