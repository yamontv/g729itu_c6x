...
/* Local functions */
static inline Word16 Gauss(Word16 *seed);
static Word16 Sqrt(Word32 Num);

static inline void update_exc_err_inl(Word32 L_exc_err[4], Word16 gain_pit, /* (i) pitch gain */
	...
	if (cur_gain == 0) {
		...
		if (L_exc_err) {
			for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR) {
			    update_exc_err_inl(L_exc_err, Gp, t0);
			}
		}
