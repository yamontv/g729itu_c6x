...
static inline void int2bin(Word16 value, Word16 no_of_bits, Word16 *bitstream);
static inline Word16 bin2int(Word16 no_of_bits, Word16 *bitstream);
 ...
 *----------------------------------------------------------------------------
 */
void prm2bits_ld8k(Word16 prm[], /* input : encoded parameters  (PRM_SIZE parameters)  */
Word16 bits[] /* output: serial bits (SERIAL_SIZE ) bits[0] = bfi
 bits[1] = 80 */
	...
	*bits++ = SYNC_WORD; /* bit[0], at receiver this bits indicates BFI */
	...
	default: {
		g729_itu_debug_out("Unrecognized frame type");
	}
 ...
 *----------------------------------------------------------------------------
 */
static inline void int2bin(Word16 value, /* input : decimal value */
 ...
 *----------------------------------------------------------------------------
 */
void bits2prm_ld8k(Word16 bits[], /* input : serial bits (80)                       */
 ...
 *----------------------------------------------------------------------------
 */
static inline Word16 bin2int( /* output: decimal value of bit pattern */
...
int read_frame(uint8_t * restrict bitvector, Word16 * restrict parm) {
	Word16 i;
	Word16 *serial = (Word16 *) bitvector; /* Serial stream               */
	int readlen = (serial[1] + 2) * sizeof(Word16);
